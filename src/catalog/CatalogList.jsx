...
export const CatalogList = ({ sortList }) => {
  ...
  useEffect(() => {
    const handleResize = () => {
      setItemsPerPage(window.innerWidth > 1000 ? 9 : 3);
      setPage(1);
    };
      ...
      <div className="catalogGoods">
        {paginatedList.length > 0 ? (
          paginatedList.map((elem) => (
            <div className="catalogGood" key={elem.id}>
              <Link to={`/product?name=${elem.title}`}>
                <img alt="good" src={require(`${elem.imageUrl}`)} />
              </Link>
              <h3>{elem.title}</h3>
              <hr />
              <h4>{elem.type}</h4>
              ...
              <div className="cartbutton">
                <svg
                  onClick={(e) => {
                    if (e.target.className.baseVal === "incart") {
                      dispatch(delCart(elem.title));
                    } else {
                      dispatch(
                        addCart(
                          paginatedList.filter(
                            (obj) => obj.title === elem.title
                          )
                        )
                      );
                    }
                  ...
                  className={
                    cart.some((obj) => obj.title === elem.title)
                      ? "incart"
                      : "cart"
                  }
                  id="svgcart "
